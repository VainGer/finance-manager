...
class TransactionModel {
    ...
    static async updateBudgets(refId, catName, oldAmount, newAmount, tDate) {
        try {
            const expenseDoc = await server_1.default.GetDocument(TransactionModel.expenseCollection, { _id: refId });
            if (!expenseDoc)
                return false;
            ...
            const profile = await server_1.default.GetDocument("profiles", { username, profileName });
            if (!profile) {
            ...
            const profileBudget = profile.budgets.find((budget) => new Date(budget.startDate) <= transactionDate && new Date(budget.endDate) >= transactionDate);
            ...
            const updatedProfileBudget = await server_1.default.UpdateDocument("profiles", { username, profileName, "budgets._id": profileBudget._id }, { $inc: { "budgets.$.spent": dif } });
            // 8. Update the matching category budget
            const updatedCategoryBudget = await server_1.default.UpdateDocument("expenses", { _id: refId }, { $inc: { "categories.$[cat].budgets.$[budget].spent": dif } }, {
                arrayFilters: [
                    { "cat.name": catName },
     ...
     */
    static async updateBudgetsOnTransactionChange(refId, catName, busName, newAmount, transactionId) {
        ...
        if (transaction.status !== 200) {
            ...
            const updatedBudgets = await this.updateBudgets(refId, catName, transaction.transaction.amount, newAmount, transaction.transaction.date);
            if (!updatedBudgets) {

...
export default function UploadTransactionsFromFile() {
    ...
    const [isCategorySuccess, setIsCategorySuccess] = useState(true);
    ...
    const Instructions = () => (
        <View className="bg-blue-100/50 p-4 rounded-lg">
            ...
            <InstructionStep number={1} bold="העלה קובץ" text="בחר קובץ Excel או CSV עם תנועות הבנק שלך" />
            <InstructionStep number={2} bold="עבד נתונים" text="המערכת תנתח ותכין את התנועות לעריכה" />
            <InstructionStep number={3} bold="ערוך ושייך" text="תוכל לערוך קטגוריות ובעלי עסקים לכל תנועה" />
            <InstructionStep number={4} bold="שמור" text="העלה את התנועות הסופיות למערכת הניהול שלך" />
        </View>
    );

    const FilePicker = () => (
        <View className="mt-6 space-y-4">
            <Text className="font-bold text-xl mb-4">בחר קובץ תנועות</Text>
            ...
            <View>
                <Button bg="#435c69" onPress={handleFileSelect}>
    ...
    const TransactionCard = ({
        ...
        return (
            <View className="bg-white p-4 m-4 rounded-lg shadow shadow-black space-y-3 border border-slate-200">
                ...
                <BusinessSelect
                    initialValue={
                        transaction.category && transaction.business ? transaction.business : ''
                    }
                    businesses={
                        transaction.category
                            ? businesses.find(b => b.category === transaction.category)?.businesses || []
            ...
            {showCreateCategory && (
                <Overlay onClose={() => setShowCreateCategory(false)}>
                    <AddCategory
                        ...
                        onCategoryAdded={() => onCategoryOrBusinessAdded(true)}
                        inOverlay
                    />
                </Overlay>
            )}
            ...
            {showCreateBusiness && (
                <Overlay onClose={() => setShowCreateBusiness(false)}>
                    <CreateBusiness
                        ...
                        onBusinessAdded={() => onCategoryOrBusinessAdded(false)}
                        categories={categories}

...
export default function useAuthProfile({ account, setProfile, setStoreProfile }) {
    ...
    const authProfile = async (profileName, pin, shouldStore = true) => {
        try {
            setLoading(true);
            setError(null);
            ...
            if (response.ok) {
                if (response.profile && typeof response.profile === 'object') {
                    setStoreProfile(shouldStore);
                    setProfile(response.profile);
                    router.replace('/home/budgetSummary');
                    return;
                } else {
                    console.error("Invalid profile in response:", response.profile);
                    setError('שגיאת שרת - נתוני פרופיל לא תקינים');
                    return;
                }
            }

...
// import { prepareImage } from '../utils/imageProcessing'; // נוסיף בהמשך
...
export default function useProfileSettings({ setLoading }) {
    ...
    const changeAvatar = async (avatar) => {
        ...
        if (avatar) {
            try {
                // TODO: נוסיף prepareImage כאשר נבנה את imageProcessing utils
                // avatarBase64 = await prepareImage(avatar.uri);
                // זמנית - נוכל להשתמש בFile reader או בצורה אחרת
                addError("העלאת תמונות עדיין לא נתמכת באתר");
                setLoading(false);
                return false;
            } catch (err) {
